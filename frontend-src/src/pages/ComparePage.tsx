...
import type { ApiKeys, CompareRequest, Provider, SuiteMetadata } from '../types'
    ...
    <div className="flex flex-col gap-2 rounded-lg border border-border bg-surface p-4">
      ...
      <select
        ...
        onChange={(e) => {
          ...
          onModelChange(MODELS_BY_PROVIDER[next][0])
        }}
      >
        {PROVIDERS.map((p) => (
...
export default function ComparePage() {
  ...
  const [providerA, setProviderA] = useState<Provider>('anthropic')
  const [modelA, setModelA] = useState(MODELS_BY_PROVIDER.anthropic[0])
  const [providerB, setProviderB] = useState<Provider>('openai')
  const [modelB, setModelB] = useState(MODELS_BY_PROVIDER.openai[0])
  ...
  const stage = useEventSource(runId)
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-3xl font-bold">Compare two models</h1>
