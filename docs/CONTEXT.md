...
| 2026-07-29 | Suite-mode `/compare` collapses a 5-item suite into one `ModelResult` per model | `cost_usd`/token counts summed across items; `latency_ms` is `p50` of all raw per-item latencies; judge metric scores averaged across items (and across repeats when `consistency_runs > 1`); `response_text` becomes a truncated per-item preview list. Coding suite skips the judge and reports `code_pass_rate` instead — `_average_judge_score` falls back to `code_pass_rate` so a winner can still be determined. |
| 2026-07-29 | Per-suite judge metric sets instead of always running all 4 | `reasoning`: correctness+completeness (no context to ground); `rag_faithfulness`: groundedness+correctness+completeness (has context+ground_truth); `safety`: safety only; `coding`: none (code_runner instead) |
| 2026-07-29 | `code_runner.py` uses `sys.executable` (falls back to `shutil.which("python")`) | Guarantees the subprocess uses the same environment/interpreter (with pytest installed) as the running server; avoids PATH-dependent failures on machines with multiple Python installs |
| 2026-07-29 | `code_runner` is process-isolation + timeout only, not a full security sandbox | No seccomp/container boundary or network firewall is implemented (matches the plan's Windows-compatible, no-`resource.setrlimit` design). Documented as a known limitation — do not point at fully untrusted code in production without adding container-level isolation. |
| 2026-07-29 | Frontend uses `HashRouter` (react-router-dom), not `BrowserRouter` | FastAPI only exposes a single `@app.get("/dashboard")` route (no catch-all for SPA sub-paths). `HashRouter` keeps all client routes under that one URL (`/dashboard#/results`, `/dashboard#/history`) so a hard refresh never 404s, with no backend routing changes required. |
| 2026-07-29 | Vite `base: "/static/"`, `build.outDir: "../frontend"` | Matches the existing `app.mount("/static", StaticFiles(directory="frontend"))` in `dashboard_server.py` — built `index.html` references `/static/assets/*.js`/`.css` correctly and `GET /dashboard` continues to `FileResponse("frontend/index.html")` unchanged. |
| 2026-07-29 | Stayed on `react-router-dom@7.18.2` despite `npm audit` flagging a "high" severity RSC-mode CSRF advisory | The advisory (GHSA-qwww-vcr4-c8h2) only affects apps using React Router's RSC/framework mode with server actions — this is a pure client-side SPA that uses neither. Downgrading to the version `npm audit fix --force` suggested (7.11.0) reintroduced multiple *other* high-severity advisories from that older release, so downgrading was strictly worse. Risk accepted and documented rather than downgraded. |
| 2026-07-29 | SSE progress requires the client to generate its own `run_id` and open `/stream/{run_id}` before POSTing `/compare` | `POST /compare` is a single synchronous request/response (not fire-and-forget), so there is no server-generated run_id to subscribe to in advance. `ComparePage.tsx` calls `crypto.randomUUID()`, opens the `EventSource` immediately, and includes the same id in the `CompareRequest.run_id` field so the backend publishes to a queue the frontend is already listening on. |
